...
def define_path(root_directory, experiment, sample):
    ...
    # in_DAPI_8x8 = f"{root_directory}/{experiment}/8x8_binned/{sample}.vsi.Collection/{sample}_40x_EFI_01/{sample}_40x_EFI_01_CH0-DAPI_mono.tif"
    # in_DAPI_BG_8x8 = f"{root_directory}/{experiment}/8x8_binned/{sample}_BG.vsi.Collection/{sample}_BG_40x_EFI_01/{sample}_BG_40x_EFI_01_CH0-DAPI_mono.tif"
...
def align_crop_BG_ROIs(in_DAPI_8x8, in_DAPI_BG_8x8, out_path, channels, in_channels, ROI_list, bounding_box_list):
    ...
    # image_ref_rgb = Image.fromarray(temp_image_ref.astype(np.uint8))
    # image_ref_rgb.show()
    ...
    # image_mov_rgb = Image.fromarray(temp_image_mov.astype(np.uint8))
    # image_mov_rgb.show()
...
#     in_channels_BG = [in_DAPI_BG, in_ATTO425_BG, in_ATTO488_BG, in_CY3_BG, in_TR_BG, in_CY5_BG, in_CY7_BG]
#
#     # # path to BG images with DAPI restaining
...
#         input = in_channels[i]
#         print(channel)
#         # crop_ROI(channel=CHANNELS[i], input=in_CHANNELS[i])
#         print("open full_scan as np array")
#         full_scan = np.array(Image.open(input))
#         print("crop ROIs")
#         for i in range(len(ROI_list)):
...
#         gc.collect()
#
#     # TODO: image registration using pyStackReg rigid-body (rotation + translation)
#
#     # 8x8 binned images are used for registration to get a transformation matrix
#     print("loading images for registration")
#
...
#             cropped_ROI = full_scan_aligned_16[bounding_box_list[i][1]:bounding_box_list[i][3],
#                           bounding_box_list[i][0]:bounding_box_list[i][2]]
#             io.imsave(f"{out_path_temp}BG_{channel}_ROI_{prefix}{ROI_list[i]}.tif", cropped_ROI)
#             # Image.fromarray(cropped_ROI).save(f"{out_path}BG_{channel}_ROI_{prefix}{ROI_list[i]}.tif", format="TIFF")
#
#             del cropped_ROI
#         del full_scan, full_scan_aligned, full_scan_aligned_16
#         gc.collect()
